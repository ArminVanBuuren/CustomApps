	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			try
			{
				var items = new List<DocumentItem>
				{
					new DocumentItem
					{
						DeliveryMethods = new List<DeliveryMethod>
						{
							...
							{
								Code = 20,
								Formats = new List<DocumentFormat>
								{
									new DocumentFormat
									{
										FormatId = 18
				...
				var items1 = items.Where(document => document.DeliveryMethods
					.Any(methods => methods.Formats
						.Any(format => format.FormatId == 25)));

				var arr = items1.OrderBy(x => x).ToArray();

				//CounteragentStatus status = CounteragentStatus.IsMyCounteragent;
				//using (var sr = new System.IO.StreamReader("C:\\test.txt", System.Text.Encoding.UTF8))
				//	if (Enum.TryParse(sr.ReadToEnd(), true, out CounteragentStatus res))
				//		status = res;
