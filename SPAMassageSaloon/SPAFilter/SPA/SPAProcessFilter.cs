...
namespace SPAFilter.SPA
{
	public class SPAProcessFilter
	{
		...
		public bool IsEnabledFilter => !ProcessPath.IsNullOrEmpty()
		                            && Directory.Exists(ProcessPath)
		                            && (!ROBPHostTypesPath.IsNullOrEmpty() && Directory.Exists(ROBPHostTypesPath) || !SCPath.IsNullOrEmpty() || File.Exists(SCPath));
		...
		void DataFilter(string filterProcess, string filterHT, string filterOp, bool bindWithFilter, ProgressCalculationAsync progressCalc)
		{
			try
			{
				...
				var scenarios = Scenarios.Intersect(fileteredOperations, new SAComparer())
					.Cast<Scenario>()
		...
		static void GetCommandsAndSubscenarios(IDictionary<string, Scenario> checkExist,
		                                       IEnumerable<Scenario> scenarios,
		                                       DistinctList<Scenario> subScenarios,
		                                       DistinctList<Command> commandsResult)
		{
		...
		static void FilterOperations(IHostType hostType,
		                             Func<IHostType, bool> neFilter,
		                             Func<IOperation, bool> opFilter,
		                             IDictionary<string, bool> allBPOperations,
		                             bool anyHasCatalogCall)
		{
		...
		public async Task AssignActivatorAsync(IEnumerable<string> filePathList)
		{
			...
			await Task.Factory.StartNew(() =>
			{
				...
				var result = MultiTasking.Run(inputFile =>
				                              ...
				                              filePathList,
				                              new MultiTaskingTemplate(filePathList.Count(), ThreadPriority.Lowest));
		...
		CollectionTemplate<ServiceInstance> GetServiceInstances(bool getValid = false)
		{
			...
			var sameHostScenarios = allInstances.SelectMany(x => x.Scenarios)
				.GroupBy(x => $"{x.HostTypeName.ToLowerInvariant()}=[{x.Name.ToLowerInvariant()}]")
				.Where(p => p.Count() > 1)
				.ToList();
			...
			{
				ReportMessage(string.Format(Resources.Filter_InitializeActivatorsWarning, string.Join(";", hrdrIDs), sameHostScenarios.Count),
		...
		static void GetScenariosAndCommands(IEnumerable<ServiceInstance> serviceInstances,
		                                    out CollectionTemplate<Scenario> resultScenarios,
		                                    out CollectionTemplate<Command> resultCommands)
		{
