...
namespace SPAFilter.SPA.Collection
{
	...
	public class ServiceCatalog : CollectionHostType
	{
		...
		public ServiceCatalog(string filePath)
		{
			...
			var allRFSCFSsList = rfsList
				.ToDictionary(x => x.Node,
				              x => XPATH.Select(navigator, $"/Configuration/CFSList/CFS[RFS/@name='{x.Node.Attributes?["name"]?.Value}']")
					              ?.Where(t => t != null)
					              .Select(p => p.Node));

			// вытаскиваем из предыдущего списка все с типом subscription
			var subscriptionRFSCFSsList = allRFSCFSsList
				.Where(p => p.Key.Attributes != null
				         && p.Key.Attributes["base"] == null
				         && p.Key.Attributes["processType"] != null
				         && p.Key.Attributes["processType"].Value.Like("Subscription"))
		...
		void GetRFS(IDictionary<XmlNode, IEnumerable<XmlNode>> collection, XPathNavigator navigator, bool isSubscriptions = false)
		{
			foreach (var rfsCFSs in collection)
			{
				...
				if (processType.Like("CancelHostType"))
					continue;
				if (CatalogRFSCollection.TryGetValue(rfsName, out var rfsOP)
				 && !rfsOP.All(p => p.Bindings.Base is CatalogRFSOperation operation && operation.RFSName.Like(rfsName)))
					throw new Exception(string.Format(Resources.ServiceCatalog_AlreadyExist, rfsName));

				var defaultLinkTypes = new List<string>
				{
					"Add",
					"Remove"
				};
		...
		CatalogRFSOperation AddBaseRFS(XmlNode node,
		                               KeyValuePair<XmlNode, IEnumerable<XmlNode>> rfsCFSs,
		                               string rfsName,
		                               string linkType,
		                               string hostType,
